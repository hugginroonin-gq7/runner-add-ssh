...
function generateSSHDConfig(config) {
  ...
  const forceCommandScript = `cd ${quoteForShellSingle(config.defaultCwd)} && if [ -n "$SSH_ORIGINAL_COMMAND" ]; then exec /bin/bash -lc "$SSH_ORIGINAL_COMMAND"; else exec /bin/bash -l; fi`;
  const forceCommandLine = !config.disableForceCwd
    ? `ForceCommand /bin/bash -lc ${quoteForShellSingle(forceCommandScript)}`
    : '';
